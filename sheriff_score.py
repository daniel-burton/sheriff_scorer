...
class PlayerScore(object):
    ...
    def total(self): #recalculate the total score, accounting for different gold value for each good type
        bread = self.bread * gold_values['bread'] #each of these calculates the gold value of the goods
        ...
        self.final_score = bread + self.bonus + cheese \
             + chicken + apple + self.contraband + self.gold_coin
...
for player in players: #get scores in each category for each player object
    ...
    player.gold_coin = int(input('How many gold coins does {} have? '.format(player.name)))
...
players.sort(key = lambda x: x.total(), reverse=True) #determine overall winner, simultaneously calling .total() on each
...
while True: #allow lookup of player score breakdown
    ...
    if len(players) > 3:
        print('BREAD: {}.\n'.format(person.bread))
    for good in ['chicken', 'apple', 'cheese', 'contraband', 'gold_coin', 'bonus']: #see, this is why a list of used goods would help
        print('{}s: {}.\n'.format(good.upper(), gold_values[good] *  getattr(person, good)))
